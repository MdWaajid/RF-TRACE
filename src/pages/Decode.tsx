      ...
      <div className="grid lg:grid-cols-3 gap-3">
        <Card title="Classification probabilities" className="lg:col-span-2">
          <div className="space-y-2">{(Object.entries(m.probs) as [string, number][]).sort((x, y) => y[1] - x[1]).map(([k, v]) => (
            <div key={k} className="flex items-center gap-3 text-sm"><span className="w-14 font-mono font-semibold text-slate-800 dark:text-slate-200">{k}</span>
              <div className="flex-1 h-3 bg-raised border border-line rounded-xs"><div className={`h-full ${k === m.detected ? 'bg-accent' : 'bg-slate-400 dark:bg-slate-600'}`} style={{ width: `${v * 100}%` }} /></div>
              <span className="w-12 text-right font-mono font-semibold text-slate-800 dark:text-slate-200">{(v * 100).toFixed(1)}%</span></div>))}</div>
        </Card>
        ...
        <Card title="CNN evidence"><ul className="text-sm space-y-1.5 list-disc pl-4 text-slate-700 dark:text-slate-300 font-medium">{m.cnnEvidence.map((e) => <li key={e}>{e}</li>)}</ul></Card>
    ...
    <Page title="Bit Stream" sub={`${bits.length} bits · ${Math.ceil(bits.length / 8)} bytes`}>
      <Card title="Recovered data" right={<div className="flex gap-2"><Btn onClick={() => setView(view === 'bin' ? 'hex' : 'bin')}>{view === 'bin' ? 'Show hex' : 'Show binary'}</Btn><Btn onClick={copy}>{copied ? 'Copied' : 'Copy'}</Btn></div>}>
        <div className="flex items-center gap-2 mb-3"><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search bit pattern, e.g. 10110"
          className="bg-raised border border-line text-slate-900 dark:text-slate-100 placeholder:text-slate-500 rounded-sm px-2 py-1.5 text-sm font-mono w-64" aria-label="Search bit pattern" />
          {q && <span className="text-xs text-slate-600 dark:text-slate-400 font-medium">{valid ? `${parts.length > 1 ? (parts.length - 1) / 2 : 0} matches` : 'Use 0 and 1 only'}</span>}</div>
        <div className="font-mono text-sm break-all leading-6 max-h-96 overflow-auto text-slate-900 dark:text-slate-100">
...
const st = { candidate: 'text-accent border-accent/40 font-semibold', inconclusive: 'text-warn border-warn/40 font-semibold', rejected: 'text-slate-600 dark:text-slate-400 border-slate-400 dark:border-slate-600' } as const;
    ...
    <Page title="FEC / Interleaving" sub="Candidate schemes ranked by score">
      <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-3">{a.result.fec.map((f) => (
        <Card key={f.name} title={f.name} right={<span className={`text-[10px] border rounded-sm px-1.5 py-0.5 ${st[f.status]}`}>{f.status}</span>}>
          <div className="font-mono text-xl font-bold text-slate-900 dark:text-slate-100">{(f.score * 100).toFixed(0)}%</div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">{f.note}</p></Card>))}</div>
    </Page>
  );
}
