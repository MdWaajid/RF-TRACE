      ...
      <div className="grid lg:grid-cols-3 gap-3">
        <Card title="Classification probabilities" className="lg:col-span-2">
          <div className="space-y-2">{(Object.entries(m.probs) as [string, number][]).sort((x, y) => y[1] - x[1]).map(([k, v]) => (
            <div key={k} className="flex items-center gap-3 text-sm"><span className="w-14 font-mono">{k}</span>
              <div className="flex-1 h-3 bg-bg border border-line"><div className={`h-full ${k === m.detected ? 'bg-accent' : 'bg-slate-600'}`} style={{ width: `${v * 100}%` }} /></div>
              <span className="w-12 text-right font-mono">{(v * 100).toFixed(1)}%</span></div>))}</div>
        </Card>
        <Card title="Result"><div className="font-mono text-3xl text-accent">{m.detected}</div><Row k="Confidence" v={`${(m.confidence * 100).toFixed(1)}%`} /></Card>
        <Card title="CNN evidence"><ul className="text-sm space-y-1.5 list-disc pl-4">{m.cnnEvidence.map((e) => <li key={e}>{e}</li>)}</ul></Card>
    ...
    <Page title="Bit Stream" sub={`${bits.length} bits · ${Math.ceil(bits.length / 8)} bytes`}>
      <Card title="Recovered data" right={<div className="flex gap-2"><Btn onClick={() => setView(view === 'bin' ? 'hex' : 'bin')}>{view === 'bin' ? 'Show hex' : 'Show binary'}</Btn><Btn onClick={copy}>{copied ? 'Copied' : 'Copy'}</Btn></div>}>
        <div className="flex items-center gap-2 mb-3"><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search bit pattern, e.g. 10110"
          className="bg-bg border border-line rounded-sm px-2 py-1.5 text-sm font-mono w-64" aria-label="Search bit pattern" />
          {q && <span className="text-xs text-slate-500">{valid ? `${parts.length > 1 ? (parts.length - 1) / 2 : 0} matches` : 'Use 0 and 1 only'}</span>}</div>
        <div className="font-mono text-sm break-all leading-6 max-h-96 overflow-auto">
          {view === 'hex' ? hex : parts.map((p, i) => (i % 2 ? <mark key={i} className="bg-accent/30 text-accent">{p}</mark> : <span key={i}>{p}</span>))}
        </div>
      </Card>
    </Page>
  );
...
const st = { candidate: 'text-accent border-accent/40', inconclusive: 'text-warn border-warn/40', rejected: 'text-slate-500 border-slate-600' } as const;
    ...
    <Page title="FEC / Interleaving" sub="Candidate schemes ranked by score">
      <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-3">{a.result.fec.map((f) => (
        <Card key={f.name} title={f.name} right={<span className={`text-[10px] border rounded-sm px-1.5 py-0.5 ${st[f.status]}`}>{f.status}</span>}>
          <div className="font-mono text-xl">{(f.score * 100).toFixed(0)}%</div>
          <div className="h-1.5 bg-bg mt-1 mb-2"><div className="h-full bg-accent" style={{ width: `${f.score * 100}%` }} /></div>
          <p className="text-xs text-slate-500">{f.note}</p></Card>))}</div>
    </Page>
  );
}
