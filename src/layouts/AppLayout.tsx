    ...
    <div className="min-h-screen flex flex-col md:flex-row">
      <aside className="md:w-56 md:min-h-screen shrink-0 bg-panel border-b md:border-b-0 md:border-r border-line flex md:flex-col overflow-x-auto">
        <div className="flex items-center gap-2 px-4 py-3 md:py-4 shrink-0">
          <svg width="22" height="22" viewBox="0 0 22 22" fill="none" stroke="#3ddbc0" strokeWidth="1.6"><path d="M1 11h4l2-7 4 14 3-10 2 3h5" /></svg>
        ...
        <nav className="flex md:flex-col md:px-2 md:pb-4 gap-0.5" aria-label="Main">
          {NAV.map((n) => (
            <button key={n} onClick={() => setPage(n)} aria-current={page === n}
              className={`text-left whitespace-nowrap text-sm px-3 py-2 rounded-sm border-l-2 ${page === n ? 'bg-raised text-accent border-accent' : 'text-slate-400 border-transparent hover:text-slate-200 hover:bg-raised/60'}`}>{n}</button>
          ))}
        </nav>
      </aside>
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="flex flex-wrap items-center gap-x-5 gap-y-2 px-4 py-2 bg-panel border-b border-line text-sm">
          ...
          <div className="flex items-center gap-2 text-slate-400"><span className={`w-2 h-2 rounded-full ${be[a.backend][0]}`} />{be[a.backend][1]}</div>
          <div className="ml-auto flex gap-2"><Btn onClick={a.reset}>New Analysis</Btn><Btn primary onClick={a.run} disabled={!a.file || a.state === 'running'}>Run Analysis</Btn></div>
        </header>
